...
export function ThemeEvidenceDetail({ contextHref, layout = "default", onApprove, onEdit, onOpenContext, onReject, theme }: ThemeEvidenceDetailProps) {
  const canApprove = theme.status !== "approved";
  ...
  return <section aria-labelledby="theme-evidence-heading" className="grid gap-5 rounded-[var(--air-radius-lg)] border border-[var(--air-color-border-default)] bg-[var(--air-color-bg-surface)] p-4 sm:p-6">
    ...
    <footer className="flex flex-wrap gap-2">{canApprove ? <Button onClick={onApprove} size="small"><Check aria-hidden="true" className="h-4 w-4" />Approve theme</Button> : null}<Button onClick={onEdit} size="small" variant="gray-subtle"><Pencil aria-hidden="true" className="h-4 w-4" />Edit theme</Button>{canReject ? <Button onClick={onReject} size="small" variant="danger-subtle"><X aria-hidden="true" className="h-4 w-4" />Reject theme</Button> : null}</footer>
  </section>;
}
