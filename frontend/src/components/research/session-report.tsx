...
import type { SessionReport as SessionReportValue, SessionReportItem as SessionReportItemValue, SessionReportItemType } from "@/domain/types";
...
export function SessionReport({ layout = "default", onApprove, onCreateRevision, onEditItem, onEditReport, onOpenContext, onRegenerate, onReview, report }: SessionReportProps) {
  ...
  return <article className="grid gap-6 rounded-[var(--air-radius-lg)] border border-[var(--air-color-border-default)] bg-[var(--air-color-bg-surface)] p-4 sm:p-6">
    ...
    {sectionOrder.map((type) => { const items = report.items.filter((item) => item.type === type); return <section className="grid gap-3" key={type}><h3 className="text-lg font-semibold">{sectionHeadings[type]}</h3>{items.length ? items.map((item) => <SessionReportItem item={item} key={item.id} layout={layout} onEdit={canEdit && onEditItem ? () => onEditItem(item) : undefined} onOpenContext={item.evidence.length && onOpenContext ? () => onOpenContext(item.id) : undefined} showActions={!isSuperseded} />) : <p className="text-sm text-[var(--air-color-text-secondary)]">No {reportItemLabels[type].toLowerCase()} items were identified.</p>}</section>; })}
