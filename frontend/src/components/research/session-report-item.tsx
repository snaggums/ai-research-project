...
export function SessionReportItem({ item, layout = "default", onEdit, onOpenContext, showActions = true, showEvidence = true }: SessionReportItemProps) {
  return <article className="grid gap-3 rounded-[var(--air-radius-md)] border border-[var(--air-color-border-default)] bg-[var(--air-color-bg-surface)] p-4">
    ...
    {showActions && (onOpenContext || onEdit) ? <footer className="flex flex-wrap gap-2">{onOpenContext ? <Button onClick={onOpenContext} size="small" variant="gray-subtle">Open evidence<ArrowUpRight aria-hidden="true" className="h-4 w-4" /></Button> : null}{onEdit ? <Button onClick={onEdit} size="small" variant="gray-subtle"><Pencil aria-hidden="true" className="h-4 w-4" />Edit</Button> : null}</footer> : null}
  </article>;
}
