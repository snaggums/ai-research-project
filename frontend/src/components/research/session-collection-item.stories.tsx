...
const meta = { title: "Research Objects/Session/Session Collection Item", component: SessionCollectionItem, tags: ["autodocs"], decorators: [(Story) => <div className="mx-auto max-w-6xl p-6" onClickCapture={(event) => { if ((event.target as HTMLElement).closest("a")) event.preventDefault(); }}><Story /></div>], args: { href: "#session", onDelete: () => undefined, onEdit: () => undefined, session: sessions[0] } } satisfies Meta<typeof SessionCollectionItem>;
export default meta;
