...
export function SessionDetailView({ activeTab, onAddParticipant, onEditParticipant, onEditSession, onRetry, projectId, projectName, routeState = "ready", session, transcriptContent, workspaceContent }: SessionDetailViewProps) {
  ...
  return <div className="grid gap-6"><PageHeader actions={onEditSession ? <Button onClick={onEditSession} size="small" variant="gray-subtle">Edit session</Button> : undefined} breadcrumbs={[{ href: "/projects", label: "Projects" }, { href: `/projects/${projectId}/overview`, label: projectName }, { href: `/projects/${projectId}/sessions`, label: "Sessions" }, { label: session.title }]} description={descriptions[activeTab]} title={session.title} /><SessionSummary session={session} /><SectionNavigation activeId={activeTab} items={sessionTabs(root)} label="Session sections" />{content}</div>;
}
