...
const participantsFor = (ids: string[]) => participantApiFixtures.filter((participant) => ids.includes(participant.id));
...
export const sessionHandlers = [
  http.get(`${SESSIONS_API_BASE_URL}/projects/:projectId/sessions`, ({ params, request }) => {
    ...
    const sessions = sessionStore.filter((session) => {
  ...
  http.post(`${SESSIONS_API_BASE_URL}/projects/:projectId/sessions`, async ({ params, request }) => {
    ...
    const session: Session = {
      ...
      theme_status: "not-generated", report_status: "not-generated", related_records: [], related_common_components: [], created_at: now, updated_at: now,
    };
  ...
  http.get(`${SESSIONS_API_BASE_URL}/projects/:projectId/sessions/:sessionId`, ({ params }) => {
    ...
    return session ? HttpResponse.json(session) : new HttpResponse("Session not found", { status: 404 });
  }),
  http.patch(`${SESSIONS_API_BASE_URL}/projects/:projectId/sessions/:sessionId`, async ({ params, request }) => {
    ...
    const payload = await request.json() as SessionPayload;
    sessionStore[index] = { ...sessionStore[index], title: payload.title, type: payload.type, starts_at: payload.starts_at ?? null, duration_minutes: payload.duration_minutes ?? null, description: payload.description ?? null, participant_ids: payload.participant_ids, participants: participantsFor(payload.participant_ids), updated_at: "2026-07-14T18:00:00Z" };
